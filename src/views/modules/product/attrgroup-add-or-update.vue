        ...
        catelogId: [
          {
            validator: (rule, value, callback) => {
              if (this.dataForm.catelogIdPath.length <= 0) {
  ...
  methods: {
        ...
        if (this.dataForm.attrGroupId) {
          this.$http({
            url: this.$http.adornUrl(
              `/product/attrGroup/${this.dataForm.attrGroupId}`
            ),
            method: "get",
            ...
            if (data) {
              ...
              this.dataForm.catelogIdPath = data.data.catelogIdPath;
            }
          });
        }
      });
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/product/attrGroup/appendOrUpdate`),
            method: "post",
            data: this.$http.adornData({
              ...
              catelogId:
                this.dataForm.catelogIdPath[
                  this.dataForm.catelogIdPath.length - 1
                ],
            }),
          }).then(({ data }) => {
            if (data) {
